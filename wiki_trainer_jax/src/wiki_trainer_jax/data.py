...
VT_E_RE = re.compile(r"\bv\s*t\s*e\b", re.IGNORECASE)
...
def looks_like_navbox(text: str, section_path: Optional[str]) -> bool:
    if section_path:
        leaf = section_path.split(">")[-1].strip().lower()
        if leaf in NAVBOX_SECTIONS:
            return True
    lower = text.lower()
    phrase_hits = sum(1 for phrase in NAVBOX_PHRASES if phrase in lower)
    if VT_E_RE.search(text):
        phrase_hits += 1
    tokens = TOKEN_RE.findall(text)
    uppercase_tokens = [tok for tok in tokens if tok.isupper() and len(tok) > 2]
    uppercase_ratio = (len(uppercase_tokens) / len(tokens)) if tokens else 0.0
    if phrase_hits >= 2:
        return True
    if uppercase_ratio >= 0.55 and len(tokens) >= 8:
        return True
    return False
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line in handle:
            ...
            for text_value, text_record in iter_text_fields(record):
                ...
                if looks_like_navbox(cleaned, section_path):
                    continue
...
def prepare_dataset(cfg: DatasetConfig) -> Dict[str, int]:
    ...
    if cfg.require_keywords:
        ...
        if missing:
            ...
            raise ValueError("required keyword(s) not found in dataset: " + ", ".join(details))
